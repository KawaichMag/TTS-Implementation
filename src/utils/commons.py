...
def mle_loss(z, m, logs, logdet, mask):
    loglike = torch.sum(logs) + 0.5 * torch.sum(
        torch.exp(-2 * logs) * ((z - m) ** 2)
    )  # neg normal likelihood w/o the constant term
    loglike = loglike - torch.sum(logdet)  # log jacobian determinant
    loglike = loglike / torch.sum(
        torch.ones_like(z) * mask
    )  # averaging across batch, channel and time axes
    loglike = loglike + 0.5 * math.log(2 * math.pi)  # add the remaining constant term
    return loglike


def duration_loss(logw, logw_, lengths):
    loglike = torch.sum((logw - logw_) ** 2) / torch.sum(lengths)
    return loglike
...
def convert_pad_shape(pad_shape):
    length = pad_shape[::-1]
    pad_shape = [item for sublist in length for item in sublist]
    return pad_shape
