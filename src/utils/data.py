...
from utils.helpers import load_wav_to_torch, load_filepaths_and_text
from utils.text import text_to_sequence, cmudict
...
class TextMelLoader(torch.utils.data.Dataset):
    ...
    def __init__(self, audiopaths_and_text, hparams):
        ...
        self.audio_dir = Path(hparams.audio_dir)
    ...
    def get_mel_text_pair(self, audiopath_and_text, dirpath=Path(".")):
        # separate filename and text
        audiopath, text = audiopath_and_text[0], audiopath_and_text[1]
        text = self.get_text(text)
        mel = self.get_mel(dirpath / audiopath)
        return (text, mel)
    ...
    def __getitem__(self, index):
        return self.get_mel_text_pair(self.audiopaths_and_text[index], self.audio_dir)
